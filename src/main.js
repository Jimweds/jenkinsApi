...
exports.init = function (host, defaultOptions, defaultParams) {
    // todo: for test
    let host = "";
    if (global.env == "online" && !isTest) {
        host = "https://tiangang:1195c7886d2949f573b8d588843876eea5@oss.foneshare.cn/jenkins";
    } else {
        host = "https://tiangang:111044f6d1365d50b37c13a8a1d369598c@jenkins.firstshare.cn";
    }
    ...
    const doRequest = function (specificOptions, customParams, callback) {
        ...
        // console.log('**********requestOptions: ', JSON.parse(JSON.stringify(requestOptions)), JSON.parse(JSON.stringify( options)))

        // Do the request
        request(requestOptions, function (error, response, body) {
            // console.log('**********after request: ', error, response && response.statusCode)
            if (error) {
                callback(error, response);
                return;
            }

            if ((Array.isArray(options.successStatusCodes) && !options.successStatusCodes.includes(
                    response.statusCode)) ||
                (Array.isArray(options.failureStatusCodes) && options.failureStatusCodes.includes(
                    response.statusCode))) {
    ...
    return {
        ...
        stop_build: function (jobName, buildNumber, customParams, callback) {
            [jobName, buildNumber, customParams, callback] = doArgs(arguments, ['string', 'string|number', [
                'object', {}
            ], 'function']);
        ...
        all_builds: function (jobName, param, customParams, callback) {
            [jobName, param, customParams, callback] = doArgs(arguments, ['string', ['string',
        ...
        update_config: function (jobName, modifyFunction, customParams, callback) {
            [jobName, modifyFunction, customParams, callback] = doArgs(arguments, ['string', 'function', [
                'object', {}
            ], 'function']);
        ...
        update_job: function (jobName, jobConfig, customParams, callback) {
            [jobName, jobConfig, customParams, callback] = doArgs(arguments, ['string', 'string', ['object',
                {}], 'function'
            ]);
        ...
        copy_job: function (jobName, newJobName, modifyFunction, customParams, callback) {
            [jobName, newJobName, modifyFunction, customParams, callback] = doArgs(arguments, ['string',
        ...
        queue_item: function (queueNumber, customParams, callback) {
            [queueNumber, customParams, callback] = doArgs(arguments, ['string|number', ['object', {}],
                'function'
            ]);
        ...
        cancel_item: function (itemId, customParams, callback) {
            ...
            doRequest({
                ...
                successStatusCodes: [HTTP_CODE_201, HTTP_CODE_204, HTTP_CODE_302],
                noparse: true
            }, customParams, callback);
        },
        ...
        create_view: function (viewName, mode, customParams, callback) {
            [viewName, mode, customParams, callback] = doArgs(arguments, ['string', ['string',
        ...
        update_view: function (viewName, viewConfig, customParams, callback) {
            [viewName, viewConfig, customParams, callback] = doArgs(arguments, ['string', 'object', ['object',
                {}
            ], 'function']);
        ...
        add_job_to_view: function (viewId, jobName, customParams, callback) {
            [viewId, jobName, customParams, callback] = doArgs(arguments, ['string', 'string', ['object', {}],
                'function'
            ]);
